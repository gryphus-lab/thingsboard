...
"no use strict";
!(function(window) {
if (window.window !== undefined && window.document)
    return;
...
window.onmessage = function(e) {
    ...
    if (msg.event && sender) {
        ...
        else if (Object.hasOwn(windowCommands, msg.command) &&
        ...
        let moduleExports = require(msg.module);
        if (!moduleExports ||
            !Object.hasOwn(moduleExports, msg.classname) ||
        ...
        if (data[0].start) {
            ...
            for (let i = 0; i < data.length; i += 2) {
                if (Array.isArray(data[i+1])) {
                    let d = {action: "insert", start: data[i], lines: data[i+1]};
                } else {
                    let d = {action: "remove", start: data[i], end: data[i+1]};
                }
...
let assert = _dereq_("assert")
let now = _dereq_("date-now")
...
function baseClone(value, isDeep, isFull, customizer, key, object, stack) {
  ...
  if (!isArr) {
    let props = isFull ? getAllKeys(value) : keys(value);
  }
...
(function (global, factory) {
  ...
  function sample(obj, n, guard) {
    ...
    let sample = isArrayLike(obj) ? clone(obj) : values(obj);
  ...
  let indexBy = group(function(result, value, key) {
    result[key] = value;
  });
  ...
  let partition = group(function(result, value, pass) {
  ...
  let difference = restArguments(function(array, rest) {
  ...
  let bind = restArguments(function(func, context, args) {
    ...
    let bound = restArguments(function(callArgs) {
      return executeBound(func, bound, context, this, args.concat(callArgs));
    });
    return bound;
  });
  let partial = restArguments(function(func, boundArgs) {
  ...
  let bindAll = restArguments(function(obj, _keys) {
    _keys = _flatten(_keys, false, false);
    let index = _keys.length;
  ...
  function collectNonEnumProps(obj, _keys) {
    ...
    let proto = isFunction(constructor) && constructor.prototype || ObjProto;
  ...
  function keys(obj) {
    ...
    let _keys = [];
    for (let key in obj) if (_has(obj, key)) _keys.push(key);
  ...
  function allKeys(obj) {
    if (!isObject(obj)) return [];
    let _keys = [];
    for (let key in obj) _keys.push(key);
    if (hasEnumBug) collectNonEnumProps(obj, _keys);
    return _keys;
  }
  function values(obj) {
  ...
  let pick = restArguments(function(obj, _keys) {
    let result = {}, iteratee = _keys[0];
  ...
  let omit = restArguments(function(obj, _keys) {
    let iteratee = _keys[0], context;
    if (isFunction(iteratee)) {
  ...
  let isArray = nativeIsArray || tagTester('Array');
  ...
  let isMap = tagTester('Map');
  let isWeakMap = tagTester('WeakMap');
  let isSet = tagTester('Set');
...
let _            = _dereq_("underscore");
_.clone          = _dereq_("lodash.clone");
let events       = _dereq_("events");
let lets         = _dereq_("./lets.js");
let messages     = _dereq_("./messages.js");
let Lexer        = _dereq_("./lex.js").Lexer;
let reg          = _dereq_("./reg.js");
let state        = _dereq_("./state.js").state;
let style        = _dereq_("./style.js");
let options      = _dereq_("./options.js");
let scopeManager = _dereq_("./scope-manager.js");
let prodParams   = _dereq_("./prod-params.js");
let console = _dereq_("console-browserify");
...
let JSHINT = (function() {
  ...
  let api, // Extension API
    ...
    member,
    membersOnly,
    predefined,    // Global letiables defined by option
  ...
  function processenforceall() {
    if (state.option.enforceall) {
      for (let enforceopt in options.bool.enforcing) {
        if (state.option[enforceopt] === undefined &&
            !options.noenforceall[enforceopt]) {
          state.option[enforceopt] = true;
        }
      }
      for (let relaxopt in options.bool.relaxing) {
  ...
  function applyOptions() {
    ...
    if (state.inES5()) {
      combine(predefined, lets.ecmaIdentifiers[5]);
    }

    if (state.inES6()) {
      combine(predefined, lets.ecmaIdentifiers[6]);
    }

    if (state.inES8()) {
      combine(predefined, lets.ecmaIdentifiers[8]);
    }
    ...
    if (state.option.couch) {
      combine(predefined, lets.couch);
    }

    if (state.option.qunit) {
      combine(predefined, lets.qunit);
    }

    if (state.option.rhino) {
      combine(predefined, lets.rhino);
    }

    if (state.option.shelljs) {
      combine(predefined, lets.shelljs);
      combine(predefined, lets.node);
    }
    if (state.option.typed) {
      combine(predefined, lets.typed);
    }

    if (state.option.phantom) {
      combine(predefined, lets.phantom);
    }

    if (state.option.prototypejs) {
      combine(predefined, lets.prototypejs);
    }
    ...
    if (state.option.devel) {
      combine(predefined, lets.devel);
    }

    if (state.option.dojo) {
      combine(predefined, lets.dojo);
    }

    if (state.option.browser) {
      combine(predefined, lets.browser);
      combine(predefined, lets.typed);
    }
    ...
    if (state.option.browserify) {
      combine(predefined, lets.browser);
      combine(predefined, lets.typed);
      combine(predefined, lets.browserify);
    }

    if (state.option.nonstandard) {
      combine(predefined, lets.nonstandard);
    }

    if (state.option.jasmine) {
      combine(predefined, lets.jasmine);
    }

    if (state.option.jquery) {
      combine(predefined, lets.jquery);
    }

    if (state.option.mootools) {
      combine(predefined, lets.mootools);
    }

    if (state.option.worker) {
      combine(predefined, lets.worker);
    }

    if (state.option.wsh) {
      combine(predefined, lets.wsh);
    }

    if (state.option.yui) {
      combine(predefined, lets.yui);
    }

    if (state.option.mocha) {
      combine(predefined, lets.mocha);
    }
  }
  function quit(code, token, a, b) {
    let percentage = Math.floor((token.line / state.lines.length) * 100);
    let message = messages.errors[code].desc;
  ...
  function removeIgnoredMessages() {
    let ignored = state.ignoredLines;
  ...
  function lintingDirective(directiveToken, previous) {
    let body = directiveToken.body.split(",")
  ...
  function peek(p) {
    let i = p || 0, j = lookahead.length, t;
  ...
  function advance(expected, relatedToken) {
    let nextToken = state.tokens.next;
  ...
  function expression(context, rbp) {
    let left, isArray = false, isObject = false;
    let initial = context & prodParams.initial;
    let curr;
  ...
  function checkComma(opts) {
    let prev = state.tokens.prev;
    let curr = state.tokens.curr;
    opts = opts || {};
  ...
  function reserve(name, func) {
    let x = type(name, func);
    x.identifier = true;
    x.reserved = true;
    return x;
  }
  function FutureReservedWord(name, meta) {
    let x = type(name, state.syntax["(identifier)"].nud);
  ...
  function relation(s, f) {
    ...
    x.led = function(context, left) {
      nobreaknonadjacent(state.tokens.prev, state.tokens.curr);
      this.left = left;
      let right = this.right = expression(context, 100);
  ...
  function findNativePrototype(left) {
    ...
    let prototype = walkPrototype(left);
    if (prototype) return walkNative(prototype);
  }
  function checkLeftSideAssign(context, left, assignToken, options) {

    let allowDestructuring = options && options.allowDestructuring;

    assignToken = assignToken || left;

    if (state.option.freeze) {
      let nativeObject = findNativePrototype(left);
      if (nativeObject)
  ...
  function assignop(s, f) {
    let x = infix(s, typeof f === "function" ? f : function(context, left, that) {
      that.left = left;
  ...
  function optionalidentifier(context, prop, preserve) {
    ...
    let curr = state.tokens.curr;
    let val  = state.tokens.curr.value;
  ...
  function identifier(context, prop) {
    let i = optionalidentifier(context, prop, false);
    if (i) {
      return i;
    }
  ...
  function parseFinalSemicolon(stmt) {
    if (state.tokens.next.id !== ";") {
      ...
      let isSameLine = sameLine(state.tokens.curr, state.tokens.next) &&
                       state.tokens.next.id !== "(end)";
      let blockEnd = checkPunctuator(state.tokens.next, "}");
  ...
  function statement(context) {
    let i = indent, r, t = state.tokens.next, hasOwnScope = false;
    ...
    if (t.id === "{") {
      let iscase = (state.funct["(verb)"] === "case" && state.tokens.curr.value === ":");
  ...
  function directives() {
    let current = state.tokens.next;
    while (state.tokens.next.id === "(string)") {
      let next = peekIgnoreEOL();
      ...
      advance();
      let directive = state.tokens.curr.value;
  ...
  function block(context, ordinary, stmt, isfunc, isfatarrow, iscase) {
    ...
    let metrics = state.funct["(metrics)"];
    ...
    if (state.tokens.next.id === "{") {
      ...
      if (isfunc) {
        ...
        let expr = expression(context, 10);
      ...
      let supportsFnDecl = state.funct["(verb)"] === "if" ||
  ...
  state.syntax["(identifier)"] = {
    ...
    nud: function(context) {
      let v = this.value;
      let isLoneArrowParam = state.tokens.next.id === "=>";
  ...
  relation("==", function(context, left, right) {
    let eqnull = state.option.eqnull &&
  ...
  prefix("new", function(context) {
    let mp = metaProperty(context, "target", function() {
      if (!state.inES6(true)) {
        warning("W119", state.tokens.prev, "new.target", "6");
      }
      let inFunction, c = state.funct;
      while (c) {
        inFunction = !c["(global)"];
        if (!c["(arrow)"]) { break; }
        c = c["(context)"];
    ...
    let c = expression(context, 155), i;
    if (c && c.id !== "function") {
      if (c.identifier) {
        switch (c.value) {
        case "Number":
  ...
  blockstmt("class", function(context) {
    let className, classNameToken;
    let inexport = context & prodParams.export;
  ...
  function classBody(classToken, context) {
    ...
    let name, accessorType, token, isStatic, inGenerator, hasConstructor;
  ...
  prefix("(", function(context, rbp) {
    let ret, triggerFnExpr, first, last;
    let opening = state.tokens.curr;
    let preceeding = state.tokens.prev;
    let isNecessary = !state.option.singleGroups;
    let pn = peekThroughParens(1);
  ...
  prefix("[", function(context) {
    ...
    let b = !sameLine(state.tokens.curr, state.tokens.next);
    this.first = [];
    if (b) {
      indent += state.option.indent;
  ...
  function functionparams(context, options) {
    ...
    let pastRest = false;
    let arity = 0;
    let loneArg = options && options.loneArg;
  ...
  function doTemplateLiteral(context, leftOrRbp) {
    ...
    let noSubst = this.noSubst;
    let depth = this.depth;
    let left = typeof leftOrRbp === "number" ? null : leftOrRbp;
    ...
    function end() {
      ...
      let complete = (state.tokens.next.template && state.tokens.next.tail &&
  ...
  function doFunction(context, options) {
    let f, token, name, statement, classExprBinding, isGenerator, isArrow,
      isMethod, ignoreLoopFunc;
    let oldOption = state.option;
    let oldIgnored = state.ignored;
    let isAsync = context & prodParams.preAsync;
    ...
    let internallyAccessibleName = !isMethod && (name || classExprBinding);
    ...
    if (!isArrow) {
      state.funct["(scope)"].funct.add("arguments", "let", token, false);
    }
    state.funct["(scope)"].stack("functionparams");

    let paramsInfo = functionparams(context, options);
  ...
  function createMetrics(functionStartToken) {
    return {
      ...
      verifyMaxComplexityPerFunction: function() {
        let max = state.option.maxcomplexity;
        let cc = this.ComplexityCount;
        if (max && cc > max) {
  ...
  (function(x) {
    x.nud = function(context) {
      let b, f, i, params, t, isGeneratorMethod = false, nextVal;
      let props = Object.create(null); // All properties, including accessors
  ...
  function destructuringPattern(context, options) {
    let isAssignment = options && options.assignment;
  ...
  function destructuringPatternRecursive(context, options) {
    let ids, idx;
    let identifiers = [];
    let openingParsed = options && options.openingParsed;
    let isAssignment = options && options.assignment;
    let recursiveOptions = isAssignment ? { assignment: isAssignment } : null;
    let firstToken = openingParsed ? state.tokens.curr : state.tokens.next;

    let nextInnerDE = function() {
      let ident;
      if (checkPunctuators(state.tokens.next, ["[", "{"])) {
        ...
        if (isAssignment) {
          let assignTarget = expression(context, 20);
          if (assignTarget) {
    ...
    let assignmentProperty = function(context) {
      let id, expr;
  ...
  function blockletiableStatement(type, statement, context) {

    let noin = context & prodParams.noin;
    let inexport = context & prodParams.export;
    let isLet = type === "let";
    let isConst = type === "const";
    let tokens, lone, value, letblock;
  ...
  let conststatement = stmt("const", function(context) {
    return blockletiableStatement("const", this, context);
  });
  ...
  let letstatement = stmt("let", function(context) {
    return blockletiableStatement("let", this, context);
  });
        ...
        if (tokens.hasOwnProperty(t)) {
          ...
          if (state.funct["(global)"] && !state.impliedClosure()) {
            if (predefined[t.id] === false) {
              ...
              if ((!state.inES5() && lets.ecmaIdentifiers[5][t.id] === false) ||
                (!state.inES6() && lets.ecmaIdentifiers[6][t.id] === false)) {
  ...
  blockstmt("function", function(context) {
    ...
    let nameToken = optionalidentifier(context) ? state.tokens.curr : null;
  ...
  prefix("function", function(context) {
    ...
    let nameToken = optionalidentifier(isAsync ? context | prodParams.async : context) ?
  ...
  blockstmt("try", function(context) {
    ...
    function catchParameter() {
      ...
      if (checkPunctuators(state.tokens.next, ["[", "{"])) {
        let tokens = destructuringPattern(context);
  ...
  (function() {
    let x = stmt("do", function(context) {
      state.funct["(breakage)"] += 1;
      state.funct["(loopage)"] += 1;
      increaseComplexityCount();

      this.first = block(context, true, true);
      let s = state.tokens.next;
      if (s.value !== "while") {
        advance("until");
      } else {
        advance("while");
  ...
  blockstmt("for", function(context) {
    ...
    let headContext = context | prodParams.noin;

    if (state.tokens.next.id === "let") {
      advance("let");
    ...
    if (_.includes(["in", "of"], nextop.value)) {
      ...
      if (nextop.value === "in" && state.option.forin) {
        if (state.forinifchecks && state.forinifchecks.length > 0) {
          let check = state.forinifchecks.pop();
  ...
  stmt("export", function(context) {
    ...
    if (state.tokens.next.type === "default") {
      ...
      advance("default");
      let exportType = state.tokens.next.id;
  ...
  function saveAccessor(accessorType, props, name, tkn, isClass, isStatic) {
    let flagName = accessorType === "get" ? "getterToken" : "setterToken";
    let key = name;
  ...
  function computedPropertyName(context) {
    ...
    let value = expression(context & ~prodParams.noin, 10);
    advance("]");
    return value;
  }
  ...
  let arrayComprehension = function() {
    let CompArray = function() {
      this.mode = "use";
      this.letiables = [];
    };
    let _carrays = [];
    let _current;
    function declare(v) {
      let l = _current.letiables.filter(function(elt) {
        if (elt.value === v) {
          elt.undef = false;
          return v;
        }
    ...
    return { stack: function() {
        ...
        unstack: function() {
          _current.letiables.filter(function(v) {
            if (v.unused)
  ...
  function lintEvalCode(internals, options, globals) {
    let priorErrorCount, idx, jdx, internal;
  ...
  let escapeRegex = function(str) {
    return str.replace(/[-\/\\^$*+?.()|[\]{}]/g, "\\$&");
  };
  let itself = function(s, o, g) {
    let x, reIgnoreStr, reIgnore;
    let optionKeys, newOptionObj, newIgnoredObj;
    ...
    combine(predefined, lets.ecmaIdentifiers[3]);
    combine(predefined, lets.reservedlets);

    declared = Object.create(null);
    let exported = Object.create(null); // letiables that live outside the current file
    ...
    let scopeManagerInst = scopeManager(state, predefined, exported, declared);
  ...
  itself.data = function() {
    ...
    let impliedGlobals = state.funct["(scope)"].getImpliedGlobals();
    ...
    let unuseds = state.funct["(scope)"].getUnuseds();
    if (unuseds.length > 0) {
      data.unused = unuseds;
    }
...
},{"./lex.js":"/../../jshint/src/lex.js","./messages.js":"/../../jshint/src/messages.js","./options.js":"/../../jshint/src/options.js","./prod-params.js":"/../../jshint/src/prod-params.js","./reg.js":"/../../jshint/src/reg.js","./scope-manager.js":"/../../jshint/src/scope-manager.js","./state.js":"/../../jshint/src/state.js","./style.js":"/../../jshint/src/style.js","./lets.js":"/../../jshint/src/lets.js","console-browserify":"/../../jshint/node_modules/console-browserify/index.js","events":"/../node_modules/events/events.js","lodash.clone":"/../../jshint/node_modules/lodash.clone/index.js","underscore":"/../../jshint/node_modules/underscore/underscore.js"}],"/../../jshint/src/lex.js":[function(_dereq_,module,exports){
...
let unicodeData = _dereq_("../data/ascii-identifier-data.js");
let asciiIdentifierStartTable = unicodeData.asciiIdentifierStartTable;
let asciiIdentifierPartTable = unicodeData.asciiIdentifierPartTable;
let nonAsciiIdentifierStartTable = _dereq_("../data/non-ascii-identifier-start.js");
let nonAsciiIdentifierPartTable = _dereq_("../data/non-ascii-identifier-part-only.js");
let es5IdentifierNames;
...
Lexer.prototype = {
  ...
  scanComments: function(checks) {
    let ch1 = this.peek();
    let ch2 = this.peek(1);
    let rest = this.input.substr(2);
    let startLine = this.line;
    let startChar = this.char;
    let self = this;
    ...
    function commentToken(label, body, opt) {
      ...
      special.forEach(function(str) {
        ...
        let strIndex = body.indexOf(str);
        if (!isSpecial && strIndex >= 0 && body.charAt(strIndex + str.length) === " ") {
          let isAllWhitespace = body.substr(0, strIndex).trim().length === 0;
          if (isAllWhitespace) {
            isSpecial = true;
        ...
        case "global":
          ...
          break;
        default:
          let options = body.split(":").map(function(v) {
  ...
  scanKeyword: function() {
    let result = /^[a-zA-Z_$][a-zA-Z0-9_$]*/.exec(this.input);
    let keywords = [
      "if", "in", "do", "let", "for", "foreach", "new",
  ...
  scanIdentifier: function(checks) {
    ...
    let readUnicodeEscapeSequence = function() {
      index += 1;
      ...
      let sequence = this.peek(index + 1) + this.peek(index + 2) +
    ...
    let getIdentifierStart = function() {
      let chr = this.peek(index);
    ...
    let getIdentifierPart = function() {
      let chr = this.peek(index);
      let code = chr.charCodeAt(0);
  ...
  scanNumericLiteral: function(checks) {
    ...
    if (char !== ".") {
      ...
      let isBigInt = this.peek(index) === 'n';
  ...
  scanEscapeSequence: function(checks) {
    ...
    case "7":
      ...
      break;
    case "u":
      let sequence = this.input.substr(1, 4);
      let code = parseInt(sequence, 16);
      if (!isHex(sequence)) {
    ...
    case "v":
      ...
      break;
    case "x":
      let  x = parseInt(this.input.substr(1, 2), 16);
  ...
  scanTemplateLiteral: function(checks) {
    ...
    while (this.peek() !== "`") {
      while ((ch = this.peek()) === "") {
        value += "\n";
        if (!this.nextLine(checks)) {
          let startPos = this.templateStarts.pop();
          this.trigger("error", {
            code: "E052",
            line: startPos.line,
      ...
      if (ch === '$' && this.peek(1) === '{') {
        ...
        let escape = this.scanEscapeSequence(checks);
        value += escape.char;
  ...
  scanStringLiteral: function(checks) {
    ...
    while (this.peek() !== quote) {
      if (this.peek() === "") { // End Of Line
        ...
        if (char === "\\") {
          let parsed = this.scanEscapeSequence(checks);
          char = parsed.char;
          jump = parsed.jump;
  ...
  scanRegExp: function(checks) {
    ...
    let malformed = false;
    let isCharSet = false;
    let isCharSetRange = false;
    let isGroup = false;
    let isQuantifiable = false;
    let hasInvalidQuantifier = false;
    let escapedChars = "";
    let hasUFlag = function() { return allFlags.indexOf("u") > -1; };
    let escapeSequence;
    let groupCount = 0;
    let terminated, malformedDesc;

    let scanRegexpEscapeSequence = function() {
      let next, sequence;
      index += 1;
      char = this.peek(index);
    ...
    let translateUFlag = function(body) {
    ...
    while (index < length) {
      ...
      if (char === "y") {
        ...
        let hasInvalidEscape = (function(groupReferences, groupCount, escapedChars, reg) {
          let hasInvalidGroup = groupReferences.some(function(groupReference) {
  ...
  next: function(checks) {
    ...
    let match = this.scanComments(checks) ||
  ...
  nextLine: function(checks) {
    ...
    let inputTrimmed = this.input.trim();
    ...
    if (!this.ignoringLinterErrors && state.option.maxlen &&
      ...
      let shouldTriggerError = !inComment || !reg.maxlenException.test(inputTrimmed);
  ...
  token: function() {
    let checks = asyncTrigger();
    let token;
    let create = function(type, value, isProperty, token) {
      let obj;
...
let errors = {
  ...
  E056: "'{a}' was used before it was declared, which is illegal for '{b}' letiables.",
...
let warnings = {
  ...
  W021: "Reassignment of '{a}', which is a {b}. " +
    "Use 'let' or 'let' to declare bindings that may change.",
  ...
  W050: "JavaScript URL.",
  W051: "letiables should not be deleted.",
  W052: "Unexpected '{a}'.",
  ...
  W083: "Functions declared within loops referencing an outer scoped " +
    "letiable may lead to confusing semantics. ({a})",
  W084: "Assignment in conditional expression",
  W085: "Don't use 'with'.",
  W086: "Expected a 'break' statement before '{a}'.",
  W087: "Forgotten 'debugger' statement?",
  W088: "Creating global 'for' letiable. Should be 'for (let {a} ...'.",
  ...
  W120: "You might be leaking a letiable ({a}) here.",
  ...
  W129: "'{a}' is defined in a future version of JavaScript. Use a " +
    "different letiable name to avoid migration issues.",
  W130: "Invalid element after rest element.",
  W131: "Invalid parameter after rest parameter.",
  W132: "`let` declarations are forbidden. Use `let` or `const` instead.",
...
let events  = _dereq_("events");
let marker = {};
let scopeManager = function(state, predefined, exported, declared) {
  ...
  let usedPredefinedAndGlobals = Object.create(null);
  let impliedGlobals = Object.create(null);
  let unuseds = [];
  ...
  let _getUnusedOption = function(unused_opt) {
  ...
  let _warnUnused = function(name, tkn, type, unused_opt) {
    let line = tkn.line;
    let chr  = tkn.from;
    let raw_name = tkn.raw_text || name;

    unused_opt = _getUnusedOption(unused_opt);

    let warnable_types = {
      "lets": ["let"],
      "last-param": ["let", "param"],
      "strict": ["let", "param", "last-param"]
    };
  ...
  function _checkForUnused() {
    ...
    let currentBindings = _current["(bindings)"];
    for (let bindingName in currentBindings) {
      if (currentBindings[bindingName]["(type)"] !== "exception" &&
        currentBindings[bindingName]["(unused)"]) {
        _warnUnused(bindingName, currentBindings[bindingName]["(token)"], "let");
      }
    }
  }
  function _checkParams() {
    let params = _current["(params)"];
    ...
    while (param) {
      let binding = _current["(bindings)"][param];
  ...
  function _getBinding(bindingName) {
    for (let i = _scopeStack.length - 1 ; i >= 0; --i) {
      let scopeBindings = _scopeStack[i]["(bindings)"];
  ...
  let scopeManagerInst = {
    ...
    unstack: function() {
      let subScope = _scopeStack.length > 1 ? _scopeStack[_scopeStack.length - 2] : null;
      ...
      let i, j, isImmutable, isFunction;
      let currentUsages = _current["(usages)"];
      let currentBindings = _current["(bindings)"];
      let usedBindingNameList = Object.keys(currentUsages);
      ...
      for (i = 0; i < usedBindingNameList.length; i++) {
        let usedBindingName = usedBindingNameList[i];

        let usage = currentUsages[usedBindingName];
        let usedBinding = currentBindings[usedBindingName];
        if (usedBinding) {
          let usedBindingType = usedBinding["(type)"];
          isImmutable = usedBindingType === "const" || usedBindingType === "import";

          if (usedBinding["(useOutsideOfScope)"] && !state.option.funcscope) {
            let usedTokens = usage["(tokens)"];
        ...
        if (subScope) {
          let bindingType = this.bindingtype(usedBindingName);
          ...
          if (!subScope["(usages)"][usedBindingName]) {
            ...
            let subScopeUsage = subScope["(usages)"][usedBindingName];
          ...
          if (typeof _current["(predefined)"][usedBindingName] === "boolean") {
            ...
            for (j = 0; j < usage["(tokens)"].length; j++) {
              let undefinedToken = usage["(tokens)"][j];
      ...
      if (!subScope) {
        Object.keys(declared)
          .forEach(function(bindingNotUsed) {
            _warnUnused(bindingNotUsed, declared[bindingNotUsed], "let");
          });
      }
      if (subScope && !isUnstackingFunctionBody &&
        !isUnstackingFunctionParams && !isUnstackingFunctionOuter) {
        let bindingNames = Object.keys(currentBindings);
        for (i = 0; i < bindingNames.length; i++) {

          let defBindingName = bindingNames[i];
          let defBinding = currentBindings[defBindingName];

          if (!defBinding["(blockscoped)"] && defBinding["(type)"] !== "exception") {
            let shadowed = subScope["(bindings)"][defBindingName];
            if (shadowed) {
    ...
    addParam: function(bindingName, token, type) {
      ...
      if (type === "exception") {
        let previouslyDefinedBindingType = this.funct.bindingtype(bindingName);
      ...
      if (_.has(_current["(usages)"], bindingName)) {
        let usage = _current["(usages)"][bindingName];
    ...
    validateParams: function(isArrow) {
      let isStrict = state.isStrict();
      let currentFunctParamScope = _currentFunctBody["(parent)"];
      let isSimple = state.funct['(hasSimpleParams)'];
      let isMethod = state.funct["(method)"];
      ...
      currentFunctParamScope["(params)"].forEach(function(bindingName) {
        let binding = currentFunctParamScope["(bindings)"][bindingName];
    ...
    getUsedOrDefinedGlobals: function() {
      let list = Object.keys(usedPredefinedAndGlobals);
    ...
    getImpliedGlobals: function() {
      let values = _.values(impliedGlobals);
      let hasProto = false;
    ...
    bindingtype: function(bindingName) {
      let scopeBindings = _getBinding(bindingName);
      if (scopeBindings) {
    ...
    addExported: function(bindingName) {
      let globalBindings = _scopeStack[0]["(bindings)"];
    ...
    addbinding: function(bindingName, opts) {
      ...
      let isblockscoped = type === "let" || type === "const" ||
        type === "class" || type === "import" || type === "generator function" ||
        type === "async function" || type === "async generator function";
      let ishoisted = type === "function" || type === "generator function" ||
        type === "async function" || type === "import";
      let isexported    = (isblockscoped ? _current : _currentFunctBody)["(type)"] === "global" &&
      ...
      if (isblockscoped) {
        ...
        let declaredInCurrentScope = _current["(bindings)"][bindingName];
        ...
        let declaredInCurrentFunctionScope = scopeManagerInst.funct.has(bindingName);
    ...
    funct: {
      bindingtype: function(bindingName, options) {
        let onlyBlockscoped = options && options.onlyBlockscoped;
        let excludeParams = options && options.excludeParams;
        let currentScopeIndex = _scopeStack.length - (options && options.excludeCurrent ? 2 : 1);
        for (let i = currentScopeIndex; i >= 0; i--) {
          ...
          let scopeCheck = excludeParams ? _scopeStack[ i - 1 ] : current;
    ...
    block: {
      ...
      use: function(bindingName, token) {
        let paramScope = _currentFunctBody["(parent)"];
        ...
        let binding = _current["(bindings)"][bindingName];
...
"use strict";
let NameStack = _dereq_("./name-stack.js");
...
exports.register = function(linter) {
  ...
  linter.on("String", function style_scanJavaScriptURLs(data) {
    let re = /^(?:javascript|jscript|ecmascript|vbscript|livescript)\s*:/i;
...
},{}],"/../../jshint/src/lets.js":[function(_dereq_,module,exports){

"use strict";

exports.reservedlets = {
  NaN       : false,
  undefined : false
};
...
let objectAssign = _dereq_('object-assign');
...
let functionsHaveNames = (function () {
...
let assert = module.exports = ok;

let regex = /\s*function\s+([^\(\s]*)\s*/;
...
assert.AssertionError = function AssertionError(options) {
  ...
  if (Error.captureStackTrace) {
    ...
    if (err.stack) {
      let out = err.stack;
      let fn_name = getName(stackStartFunction);
      let idx = out.indexOf('\n' + fn_name);
      if (idx >= 0) {
        let next_line = out.indexOf('\n', idx + 1);
...
function _deepEqual(actual, expected, strict, memos) {
  if (actual === expected) {
    ...
    let actualIndex = memos.actual.indexOf(actual);
    if (actualIndex !== -1) {
...
function _throws(shouldThrow, block, expected, message) {
  ...
  let userProvidedMessage = typeof message === 'string';
  let isUnwantedException = !shouldThrow && util.isError(actual);
  let isUnexpectedException = !shouldThrow && actual && !expected;
...
function EventEmitter() {
  if (!this._events || !Object.hasOwn(this, '_events')) {
...
function objectKeysPolyfill(obj) {
  let keys = [];
  for (let k in obj) if (Object.hasOwn(obj, k)) {
    keys.push(k);
  }
  return k;
}
...
let oop = require("../lib/oop");
let Mirror = require("../worker/mirror").Mirror;
let lint = require("./tbel/jshint").JSHINT;

function startRegex(arr) {
    return RegExp("^(" + arr.join("|") + ")");
}

let disabledWarningsRe = startRegex([
    "Bad for in letiable '(.+)'.",
    'Missing "use strict"'
]);
let errorsRe = startRegex([
    "Unexpected",
    "Expected ",
...
let TbelWorker = exports.TbelWorker = function(sender) {
